...
function isWildcardEnabled(key) {
...
export function scrapeLinkedInProfile() {
  ...
  try {
    ...
    // Only scrape if any name-related wildcard is enabled
    if (isWildcardEnabled('firstName') || isWildcardEnabled('lastName') || isWildcardEnabled('fullName')) {
      let fullName = '';
    ...
    // Get headline - only if enabled
    if (isWildcardEnabled('headline')) {
    ...
    // Get company name and position - only if enabled
    if (isWildcardEnabled('companyName') || isWildcardEnabled('company') || isWildcardEnabled('position')) {
      ...
      if (companyButton) {
        ...
        if (match && isWildcardEnabled('companyName')) {
          data.companyName = match[1].trim();
        }
      }

      // Fallback: Get from experience section
      if ((!data.companyName || !data.position) && (isWildcardEnabled('companyName') || isWildcardEnabled('position'))) {
        ...
        if (experienceSection) {
          ...
          if (firstItem) {
            ...
            // Look for company
            if (isWildcardEnabled('companyName')) {
    ...
    // Get message recipient name from open message conversation - only if enabled
    if (isWildcardEnabled('msgFirstName') || isWildcardEnabled('msgLastName') || isWildcardEnabled('msgFullName')) {
